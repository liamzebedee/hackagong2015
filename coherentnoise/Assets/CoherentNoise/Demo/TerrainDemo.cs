...
public class TerrainDemo : MonoBehaviour
{
    ...
    public float Speed = 70; // flight speed
    ...
    void Start()
    {
        ...
        var desert = new Gain(
            new RidgeNoise(1242132141)
                {
                    OctaveCount = 8
                } * 0.6f, 0.4f);
        // hills use a simple pink noise. 
        var hills = new PinkNoise(12321)
                        {
                            OctaveCount = 6, // smooth hills (no higher frequencies)
    ...
    void Update()
    {
        if (m_Move)
        {
            ...
            if (false) //m_CameraChangingHeight || Mathf.Abs(targetheight - nextPoint.y) > 10)
            {
    ...
    private IEnumerator CreateMapsAtStart()
    {
		...
		Console.WriteLine ("Terrain 1 gen'd");
        m_NoiseCoord++;
		...
		Console.WriteLine ("Terrain 2 gen'd");
        m_Move = true;
    }
	...
	public float MinkowskiExponent=2;
	
	public float Minkowski(Vector3 vec)
	{
		return Minkowski(vec.x,vec.y,vec.z);
	}
	public float Minkowski(Vector2 vec)
	{
		return Minkowski(vec.x,vec.y);
	}
	/// <summary>
	/// Minkowski Distance, i.e. the generalized p-norm of the difference.
	/// </summary>
	public float Minkowski(float x, float y, float z)
	{
		//if (p < 0) throw new ArgumentOutOfRangeException("p");
		//if (p == 1) return Manhattan(a, b);
		//if (p == 2) return Euclidean(a, b);
		//if (float.IsPositiveInfinity(p)) return Chebyshev(a, b);
		x=Math.Abs(x);
		y=Math.Abs(y);
		z=Math.Abs(z);
		
		var norm=Math.Pow(Math.Pow(x,MinkowskiExponent)+Math.Pow(y,MinkowskiExponent) ,1/ MinkowskiExponent);
		
		return (float)Math.Pow(Math.Pow(norm, MinkowskiExponent)+Math.Pow(z,MinkowskiExponent), 1 / MinkowskiExponent);
	}
	public float Minkowski(float x, float y)
	{
		...
		var norm=Math.Pow(Math.Pow(x,MinkowskiExponent)+Math.Pow(y,MinkowskiExponent) ,2/MinkowskiExponent);
		//add luminosity parameter instead of 3
		return (float)Math.Pow(Math.Pow(norm, MinkowskiExponent),2/MinkowskiExponent);
	}
}
